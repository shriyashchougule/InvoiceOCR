...
#hf_images = h5py.File('ValidationImagesz4.h5', 'w')
#hf_masks = h5py.File('ValidationMasks4.h5', 'w')
...
def add_table_headings(img):
    ...
    font                   = random.choice(cv2fontList) 
    fontScale              = random.uniform(0.5, 0.75)
    fontColor              = random.uniform(30, 120)
    lineType               = 2
...
while index < 2000:
    for tl in templateList:
        ...
        gamma = max(np.random.normal(0.08, 0.2),0.04)
        ...
        gauss = np.random.normal(0, 0.85,tlcrop.size)
        ...
        # Generate Gaussian noise
        noiseVar = max(np.random.normal(60, 200), 10)        
        ...
        #print(tlcrop.shape, maskcrop.shape)        
        cv2.imshow("image", tlcrop)
        cv2.imshow("mask", maskcrop*60)
        #cv2.imshow("image2", tl)
        cv2.waitKey(0)
        cv2.imwrite("filename"+str(index)+".png", tlcrop)
 
        print(index)
        # Save in HDF5 format
        #hf_images.create_dataset(name = str(index), data = tlcrop, shape = (615, 1280), compression="gzip", compression_opts=9)
        #hf_masks.create_dataset(name = str(index), data = maskcrop, shape = (615, 1280), compression="gzip", compression_opts=9)
        index += 1
        #hf_images.flush()
        #hf_masks.flush()
#hf_images.close()
